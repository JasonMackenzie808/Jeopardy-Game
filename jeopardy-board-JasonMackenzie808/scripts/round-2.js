...
async function fetchJeopardyQuestions() {
  ...
  try {
    ...
    let placeholderQuestions = data["double jeopardy"]
    placeholderQuestions.forEach((question) => {
      if (question.category === "NATIONAL GEOGRAPHIC TREASURES OF EGYPT") {
        category1Questions.push(question);
      } else if (question.category === "BROADWAY") {
        category2Questions.push(question);
      } else if (question.category === "PRESIDENTIAL SPEECHES") {
        category3Questions.push(question);
      } else if (question.category === "FRENCH LITERATURE") {
        category4Questions.push(question);
      } else if (question.category === "MEASURING UP") {
        category5Questions.push(question);
      } else if (question.category === "PHRASES OF HELLO & GOODBYE") {
...
console.log("it's working from round-2");
...
//Round 2 Next Page Button
let nextRoundButton2 = document.getElementById("next-round-btn2");

nextRoundButton2.addEventListener("click", () => {
  document.location = `./final-round.html?round=final&player1=${player1Name}&player2=${player2Name}&player1score=${player1CurrentScore}&player2score=${player2CurrentScore}&playerturn=${currentPlayerTurn}`;
});
...
function checkScores() {
  if (
    parseInt(player1CurrentScore) >= 30000 ||
    parseInt(player2CurrentScore) >= 30000
  ) {
    cards.forEach((cell) => {
    ...
    window.alert(`Continue to the next round.`);
    nextRoundButton2.disabled = false;
  }
}
